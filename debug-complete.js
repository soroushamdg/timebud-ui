// Complete test of the fixed planner
const URGENCY_K = 3.5;
const PRI = 2.0;
const DEF = 1.0;
const SOLO_PRI = 2.0;
...
console.log('=== COMPLETE FIXED PLANNER TEST ===');
...
// Sort by final position (in this case, just the order they were added)
allScheduled.forEach((task, index) => {
  console.log(`${index + 1}. ${task.title} from ${task.projectName}: ${task.scheduledMinutes} minutes`);
});

console.log('\n=== CONCLUSION ===');
if (allScheduled.length === 1) {
  console.log(`Only one task scheduled: ${allScheduled[0].title}`);
  console.log('This happens because both projects have equal weights and split the budget,');
  console.log('but only one task fits in the remaining budget after allocation.');
} else if (allScheduled.length === 2) {
  console.log('Both tasks scheduled with partial allocation');
} else {
