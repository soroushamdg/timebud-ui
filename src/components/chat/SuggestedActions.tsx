    ...
    <div className="flex gap-2 mt-2 overflow-x-auto scrollbar-hide pb-1 -mx-1 px-1" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', WebkitOverflowScrolling: 'touch' }}>
      {displayedSuggestions.map((suggestion, idx) => (
        <button
          ...
          className="bg-bg-card border border-border-card text-white text-sm px-4 py-2 rounded-full hover:border-accent-yellow transition-colors whitespace-nowrap flex-shrink-0"
        >
          {suggestion.label}
        </button>
      ))}
