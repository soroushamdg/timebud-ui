  ...
  return (
    <div className="pt-3 pb-4">
      <div className="flex items-center justify-between mb-2 px-5">
        <h2 className="text-white text-sm font-bold">Week ahead</h2>
        <button
          onClick={() => router.push('/planner')}
          className="text-text-sec text-xs font-medium hover:text-white transition-colors"
        >
          See full week
        </button>
      </div>

      <div className="flex gap-2 overflow-x-auto scrollbar-hide px-5 pb-1 snap-x">
        {days.map((day, index) => {
          ...
          return (
            <button
              ...
              className={`flex-shrink-0 min-w-[64px] snap-start rounded-2xl px-2 py-3 flex flex-col items-center gap-1.5 transition-colors ${
                isToday ? 'bg-black border border-accent-yellow' : 'bg-black border border-transparent'
              }`}
            >
              ...
              <span className="text-text-sec text-[10px]">{day.taskCount} jobs</span>
            </button>
          )
        })}
      </div>
      ...
      {unscheduledCount > 0 && (
        <div className="mx-5 mt-3 bg-black border border-[#2a2a2a] rounded-xl px-4 py-3 flex items-center justify-between gap-3">
          <p className="text-text-sec text-sm">
            {unscheduledCount} job{unscheduledCount === 1 ? '' : 's'} won&apos;t fit in the next 7 days at this pace.
          </p>
          <button
            onClick={() => router.push('/planner')}
