...
export function ErrorDialog({ isOpen, title, message, onDismiss }: ErrorDialogProps) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-scrim/50 z-[200]" onClick={onDismiss} />
      ...
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-bg-card rounded-2xl z-[210] max-w-sm w-full mx-4">
        ...
        <div className="flex items-center justify-between p-4 border-b border-border-card">
          ...
          <button
            onClick={onDismiss}
            className="w-6 h-6 rounded-lg bg-bg-primary flex items-center justify-center text-text-sec hover:text-text-primary transition-colors"
          >
            <X size={14} />
          </button>
        </div>
        ...
        <div className="p-4 pt-0">
          <button
            onClick={onDismiss}
            className="w-full bg-accent-pink text-on-dark-accent font-medium py-3 rounded-xl hover:bg-opacity-90 transition-opacity"
          >
            OK
          </button>
        </div>
