...
export default function ChatPage() {
  ...
  const handleSendMessage = async (content: string, files?: any[]) => {
    ...
    try {
      ...
      if (data.toolsExecuted && data.toolsExecuted.length > 0) {
        ...
        const hasDestructive = data.toolsExecuted.some(t => 
          t.tool.includes('delete') || t.tool.includes('remove')
        )
      ...
      setError({
        code: 'network_error',
        message: 'Failed to connect to AI. Please check your connection and try again.',
      })
      setLoading(false)
    }
  }
  ...
  const handleExplainMore = () => {
    const message = messages.find(m => m.id === quickActionMessageId)
    if (message) {
      handleSendMessage(`Can you explain more about: "${message.content.substring(0, 100)}..."`)
    }
  }
  ...
  const getEmptyStateSuggestions = () => {
    ...
    const timeBasedSuggestion = hour < 12 
      ? 'Plan my day' 
      : hour < 17 
        ? 'Review my progress' 
    ...
    <AppShell showTabBar={true}>
      <div className="flex flex-col h-[calc(100vh-6rem)] bg-bg-primary">
          ...
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-6">
              ...
              <p className="text-text-sec mb-6">
                I'm your AI assistant. Ask me anything about your projects and tasks.
              </p>
        ...
        <QuickActionSheet
          ...
          isPinned={messages.find(m => m.id === quickActionMessageId)?.isPinned || false}
        />
