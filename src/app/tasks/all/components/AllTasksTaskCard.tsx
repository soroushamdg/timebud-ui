  ...
  return (
    <div className="flex items-center gap-3 min-w-0">
      <div
        ...
        className={`flex-1 min-w-0 bg-bg-card rounded-2xl px-4 py-3 flex items-center gap-3 border border-card-border-strong cursor-pointer transition-all hover:bg-bg-card-hover min-h-[72px] relative shadow-[0_4px_16px_rgba(0,0,0,0.35)] ${
      ...
      >
        ...
        <input
          ...
          className="w-6 h-6 rounded-md flex-shrink-0 accent-accent-yellow cursor-pointer"
        />
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <h4 className="text-text-primary text-base font-semibold truncate min-w-0">
              {task.title}
            </h4>
          ...
          <div className="flex items-center gap-2 mt-1">
            {task.due_date && (
              <>
                <CalendarIcon className={`w-3 h-3 flex-shrink-0 ${taskIsOverdue && !completed ? 'text-status-overdue' : taskIsToday && !completed ? 'text-status-today' : 'text-text-sec'}`} />
                <span className={`text-sm truncate ${taskIsOverdue && !completed ? 'text-status-overdue font-semibold' : taskIsToday && !completed ? 'text-status-today font-semibold' : 'text-text-sec'}`}>
        ...
        <div className="flex-shrink-0 flex flex-col items-end gap-1 px-2">
          ...
          {!completed && (
            <span className="text-[10px] font-bold text-on-light-accent bg-accent-yellow px-2 py-0.5 rounded-full shadow-[0_0_8px_rgba(255,210,51,0.45)]">
        ...
        <div className="relative">
          <button
            ...
            className="p-1.5 rounded-xl hover:bg-bg-card-hover transition-colors"
          >
          ...
          {showMenu && (
            <div className="absolute right-0 top-full mt-2 w-44 bg-bg-card border border-border-card rounded-2xl shadow-[0_8px_24px_rgba(0,0,0,0.5)] z-50 overflow-hidden p-1">
              <button
                onClick={handleEdit}
                className="w-full px-3 py-2.5 text-left text-text-primary hover:bg-bg-card-hover rounded-xl transition-colors flex items-center gap-2"
              >
                <Edit size={16} />
                Edit
              </button>
              <button
                ...
                className="w-full px-3 py-2.5 text-left text-accent-yellow hover:bg-accent-yellow/10 rounded-xl transition-colors flex items-center gap-2"
              >
