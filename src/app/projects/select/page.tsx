    ...
    <div className="min-h-screen bg-black text-white">
            ...
            <div className="grid grid-cols-2 gap-4">
              {activeAndPausedProjects.map(project => (
                ...
                >
                  ...
                  {/* Active state badge */}
                  {isActive(project) && (
                    <div className="absolute bottom-1 right-1 bg-accent-yellow rounded-full w-5 h-5 flex items-center justify-center">
          ...
          <div className="px-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {archivedProjects.map(project => (
                <div
                  key={project.id}
                  className="rounded-2xl overflow-hidden relative cursor-pointer filter grayscale(100%) opacity-60"
                  onClick={() => handleProjectTap(project.id, getProjectStatus(project))}
                  onTouchStart={(e) => handleLongPressStart(e, project.id)}
                  onMouseDown={(e) => handleLongPressStart(e, project.id)}
                  onTouchEnd={handleLongPressEnd}
      ...
      {contextMenu.projectId && (() => {
        ...
        return (
          <div
            ref={contextMenuRef}
            className="fixed bg-bg-card border border-border-card rounded-lg py-2 z-50"
            style={{
              left: contextMenu.x,
              top: contextMenu.y,
            }}
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center">
          <div className="bg-bg-card rounded-2xl p-6 max-w-sm mx-4">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 py-2 border border-border-card rounded-lg text-white"
              >
                Cancel
              </button>
              <button
                onClick={confirmDelete}
                className="flex-1 py-2 bg-accent-pink rounded-lg text-white"
              >
                Delete
              </button>
            </div>
