...
declare
  ...
  result jsonb;
begin
  select user_id into test_user_id from user_ai_settings limit 1;
  if test_user_id is null then
    return jsonb_build_object('error', 'no user_ai_settings row found to test against');
  end if;

  select xp_total into xp_before from user_ai_settings where user_id = test_user_id;
  ...
  update tasks set status = 'completed' where id = test_task_id;
  select xp_total into xp_after_complete from user_ai_settings where user_id = test_user_id;
  select mission_bonus_awarded into bonus_awarded_after from projects where id = test_project_id;

  update tasks set status = 'pending' where id = test_task_id;
  select xp_total into xp_after_undo from user_ai_settings where user_id = test_user_id;
