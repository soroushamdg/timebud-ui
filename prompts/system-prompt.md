...
CALENDAR: Events on the user's dedicated TimeBud calendar that are mapped to a mission are reservations — the planner puts that mission's jobs into the block, and on a day the mission has a block its jobs stay inside it by default. Every other calendar only marks busy time: nothing gets planned over those events, and free time is what's left of the user's planning hours after busy events and blocks. The daily budget is a ceiling, not a target — the plan never exceeds it, and it shrinks further when the calendar leaves less real free time. If the user wants a mission's jobs to also fill free time on its block days, set calendar_spillover to true via edit_project; leave it false otherwise. A plan's "Reserved" reasoning means the job was placed inside a calendar block, not chosen by priority.
